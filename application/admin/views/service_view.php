    ...
    <div class="row">
      <div class="col-md-12 col-sm-12 col-xs-12">
        <div class="x_panel">
          ...
          <div class="x_content">
            <div class="res"></div>
              <div class="col-md-12"><a class="btn btn-success col-md-3 pull-right" href="<?php echo site_url('service/add_service') ?>">Add Services</a></div>
              <table id="item-list" class="table table-striped table-bordered">
                <thead>
                  <tr>
                    <th>ServiceID</th>
                    <th >Service name</th>
                    <th >Parent/Sub Category/Child</th>
                    <th>Price</th>
                    <th>Est.Duration[time]</th>
                    <th >UserID</th>
                    <th>UserName</th>
                     <th >ShopID</th>
                    <th>ShopName</th>
                  ...
                  foreach ($service_list as $value) {
                    ...
                    <td class="cls-td">
                      ...
                      <?php if($admin_data[0]['user_promotion'] != 1){
                              if(!empty($user_access_data)){
                                if($user_access_data[0]['u_delete'] == 1){?>
                                  <button type="button" name="button" class="cls-btn-active-class btn cls_delete_btn"   data-shop-id="<?=$value->id?>" style="padding:5px 17px !important;background: #40E0D0;border-radius:3px;border:none;color:white;" >Delete</button>
                      <?php }}}else{?>
                                <button type="button" name="button" class="cls-btn-active-class btn cls_delete_btn"   data-shop-id="<?=$value->id?>" style="padding:5px 17px !important;background: #40E0D0;border-radius:3px;border:none;color:white;" >Delete</button>
                      <?php }?>
                    </td>
...
function(isConfirm) {
  if (isConfirm) {
    $.ajax({
        url: site_url + 'service/remove_service',
        method: "POST",
        data: {id: shop_id},
        async: false,
        success: function (data) {
          console.log(data);
            ...
            if (obj.success == 'success') {
                $('#row_' + obj.id).remove();
                $('.res').html('<div class="alert alert-success alert-dismissable"><button aria-hidden="true" data-dismiss="alert" class="close" type="button">×</button>Service deleted successfully.</div>');
            }
            else if (obj.unsuccess == 'unsuccess') {
                $('.res').html('<div class="alert alert-danger alert-dismissable"><button aria-hidden="true" data-dismiss="alert" class="close" type="button">×</button>unsuccessfully!</div>');
