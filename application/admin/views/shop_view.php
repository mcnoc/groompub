            ...
            <?php }?>
              ...
              <tbody>
                  ...
                  if($value->user_id == 0){
                    ...
                    <td><?php if(isset($value->changed_shop_name)){ echo $value->changed_shop_name;}?></td>
                    <td><?php if(isset($value->shop_email)){ echo $value->shop_email;}?></td>
                    <td><?php if(isset($value->mobile)){ echo $value->mobile;}?></td>
                     <td><?php if(isset($value->varification)){ if( $value->varification==0){ echo 'Yes';} elseif( $value->varification==1){ echo 'No';}   }?></td>
                    <td class="cls-td">
                      <?php if($admin_data[0]['user_promotion'] != 1){
                      ...
                      <?php }}}else{?>
                                <button type="button" name="button" class="cls-btn-active-class btn cls_delete_btn"  data-shop-id="<?=$value->id?>" style="padding:5px 10px 5px 10px !important;width:70px;background: #40E0D0;border-radius:3px;border:none;color:white;" >Delete</button>
                      <?php }?>
                      <?php if($value->is_shop_name_approved == '1') { ?>
                        <br><button type="button" class="approve_shop_name btn btn-primary" data-shop_id="<?= $value->id ?>" data-shop_new_name="<?= $value->changed_shop_name ?>" style="width: 227px;margin-top: 2px;">Approve New Shop Name</button>
                      <?php }?>
                    </td>
                    </tr>
                  <?php }?>
...
$(document).on('click', '.approve_shop_name', function(){
  var shop_id = $(this).attr('data-shop_id');
  var shop_new_name = $(this).attr('data-shop_new_name');
  // console.log(shop_id);
  $.ajax({
    url: site_url + 'shop/update_new_shop_name',
    method: 'POST',
    data: {id: shop_id, shop_new_name: shop_new_name},
    ...
    $.ajax({
        ...
        success: function (data) {
          console.log(data);
            ...
            if (obj.success == 'success') {
                $('#row_' + obj.id).remove();
                $('.res').html('<div class="alert alert-success alert-dismissable"><button aria-hidden="true" data-dismiss="alert" class="close" type="button">×</button>Page deleted successfully.</div>');
            }
            else if (obj.unsuccess == 'unsuccess') {
                $('.res').html('<div class="alert alert-danger alert-dismissable"><button aria-hidden="true" data-dismiss="alert" class="close" type="button">×</button>unsuccessfully!</div>');
