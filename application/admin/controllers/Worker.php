...
class Worker extends Admin_Controller {
    ...
    public function index() {
        ...
        if ($this->session->userdata('admin_id')) {
          ...
          foreach ($worker_list as $key => $value) {
            if($value->user_id != 0){
                $user_list1 = $this->general_model->get_user_by_shop( 'user', array('email' => $value->email));
                $worker_list[$key]->emaill = $user_list1;
               // print_r($user_list1);
                
                $user_listt = $this->general_model->get_user_by_shop_document( 'document', array('is_deleted' => 0,'user_id' => $user_list1->userid));
                $worker_list[$key]->document = $user_listt;
 ...
 public function active_user_id_shopmanager() {
      $data = array();
      if ($_POST['id'] != '') {
         $sid = $_POST['id'];
          $data = array(
    ...
    public function active_user_id_shopmanager_dlt() {
      $data = array();
      if ($_POST['id'] != '') {
         $sid = $_POST['id'];
          $data = array(
 ...
 public function active_user_id_lice() {
      $data = array();
      if ($_POST['id'] != '') {
        $sid = $_POST['id'];
          $data = array(
