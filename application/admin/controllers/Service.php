            ...
            // echo '<pre>';print_r($service_list);exit;
              ...
              if($value->worker_id != 0){
                  ...
                  foreach ($worker as $key1 => $item) {
                    $worker_list = $this->general_model->get_service_by_service( 'workers', array('id' => $item , 'is_deleted' => 0));
    ...
    public function service_info($id1) {
      ...
      if ($this->session->userdata('admin_id')) {
        ...
        $id1 = $this->url_decrypt($id1);
          if ($id1 != '') {
              $service_data = $this->general_model->get_services_data('services.*', 'services', array('id' => $id1,'is_deleted'=>0));
