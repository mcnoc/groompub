...
class Dashboard extends Admin_Controller {
    ...
    public function index() {
        ...
        if ($this->session->userdata('admin_id')) {
           ...
           // $user_val = $this->general_model->get_all_general_data("*", "user", array('is_deleted' => 0), 'result_array');
            $user_val = $this->general_model->get_all_general_data("*", "user");
            $this->data['user_val'] = count($user_val);

            $category_list = $this->general_model->get_all_general_data("*", "category", array('is_deleted' => 0, 'parent_id' => 0), 'result_array');
    ...
    //   $this->data['admin_data'] = $admin_data;
        ...
        if ($this->input->post()) {
            ...
            if ($this->form_validation->run() == false) {
                $this->session->set_flashdata('error_message', "Email id is require");
...
//                    echo $ChangePassUrl;exit;
                    ...
                    $this->email->from('info@gggroom.com', 'GGG Rooms');
