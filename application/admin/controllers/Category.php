...
class Category extends Admin_Controller {
    ...
    public function insert_child_category() {
        if ($this->session->userdata('admin_id')) {
              ...
              if ($this->form_validation->run() == false) {
                ...
                $service_name = $this->input->post('parent_category');
                $sub_category = $this->input->post('sub_category');
                $cat = $this->general_model->check_exist_data('category_id', 'category', array('cat_name' => $category_name,  'parent_id' => $sub_category, 'flag' => 2,'is_deleted' => 0));
                if(!empty($cat)){
                  $this->session->set_flashdata('error_message', "Category already exist.");
                  redirect('category/add_child_category', 'refresh');
                }
	...
	public function edit_multiple_category($cat_ids) {
      if ($this->session->userdata('admin_id')) {
          $cat_arr = explode('A', $cat_ids);
          // echo '<pre>';print_r($cat_arr);die;
          $admin_id = $this->session->userdata('admin_id');
          $admin_data = $this->general_model->get_all_general_data("*", "admin", array('is_deleted' => 0, 'id' => $admin_id), 'result_array');
          $this->data['admin_data'] = $admin_data;
          $this->data['cat_ids'] = $cat_ids;

          foreach ($cat_arr as $index => $cat_id) {
              if($cat_id != '') {
                  $cat_data = $this->general_model->get_category_all_data_list('*', 'category', array('category_id' => $cat_id));
                  $this->data['category_list'][$index] = $cat_data;
              }
          }
          $this->render('edit_multiple_category_view');
      } else {
          redirect('Login', 'refresh');
      }
    }

    public function update_mutiple_category() {
      if ($this->session->userdata('admin_id')) {
          // echo '<pre>';print_r($_POST);die;
          $this->load->helper('form','url');
          $this->load->library('form_validation');
          $cat_name_arr = $this->input->post('category_name');
          $cat_id_arr = $this->input->post('category_id');
          ...
          if ($this->form_validation->run() == false) {
              $this->session->set_flashdata('error_message', "Please fill required fields.");
              // $this->session->set_userdata('CATEGORY_DETAIL', $_POST);
              redirect('category/edit_multiple_category/'.$cat_ids);

          } else {
              foreach ($cat_id_arr as $index => $cat_id) {
                  $cat_name = $cat_name_arr[$index];
                  $where = array('category_id !=' => $cat_id, 'cat_name' => $cat_name, 'is_deleted' => 0);
                  $cat_count = count($this->db->from('category')->where($where)->get()->result_array());
                  // echo $cat_count;
                  ...
                  if($cat_count == 0) {
                      ...
                      $this->general_model->update_category_data($data, 'category', array('category_id' => $cat_id));
                  } else {
                      $this->session->set_flashdata('error_message', "Duplicate category not allowed");
                      redirect('category/edit_multiple_category/'.$cat_ids);
                  }
              }
...
public function update_category() {
      if ($this->session->userdata('admin_id')) {
            ...
            if ($this->form_validation->run() == false) {
              ...
              redirect('category/add_category', 'refresh');
                ...
                $where= array('category_id !=' => $category_main_id, 'cat_name' => $category_name, 'is_deleted' => 0);
                $cat_count = count($this->db->from('category')->where($where)->get()->result_array());
    ...
    public function update_category1() {
      if ($this->session->userdata('admin_id')) {
            $this->load->helper('form','url');
            $this->load->library('form_validation');
            $this->form_validation->set_rules('category_name', 'Category name', 'required');
