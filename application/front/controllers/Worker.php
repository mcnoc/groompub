      ...
      if (!$this->session->userdata('uid')) {
        ...
        if($user_list->u_category == 2 || $user_list->u_category == 3){
          ...
          if ($this->input->post()) {
             ...
             //$this->form_validation->set_rules('worker_email', 'Worker Email', 'required|valid_email|is_unique[user.email]');
          ...
          $uid = $uidd;
               ...
               // $worker_permission = '3';
                ...
                if($worker_permission == '1'){
                  ...
                  $emailsend = $this->general_model->send_worker_permission_email_w($u_data);
                }
            ...
            //  $this->form_validation->set_rules('worker_email', 'Worker Email', 'required|valid_email');
              ...
              if ($this->form_validation->run() == false) {
                ...
                $data = array(
                    ...
                    'service_id' => $chk2,
                    'user_id' => $uid,
                    'shop_permission' => $worker_permission,
                    'image' => $image,
                    'created_at' => date('Y-m-d H:i:s'),
                    'updated_at' => date('Y-m-d H:i:s'),
                    'is_active' => 1,
    ...
    public function check_shop_hours_for_worker() {
     ...
     public function delete_vacation_module_time() {
      ...
      public function get_worker_shop_services()
      {
       ...
       // $main_filter_shop_list = $this->db->select('shop.*')->from('shop')->where('shop.is_deleted','0')->where('shop.user_id','!=','0')->where('shop.service_id','!=','')->where('shop.id',$all_shop_id[$x])->get()->result();
       ...
       public function check_vacation_module_end_time() {
        ...
        //   );
         ...
         // $this->session->set_flashdata('success_message', "Congratulation!, Your permission is active.");
          $this->session->set_flashdata('success_message', "Congratulation!, GO To document page after login and upload Government issued ID.");
