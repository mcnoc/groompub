   ...
   public function get_city()
      {
       ...
       $all_shop_id = $this->input->post('state_id');

        $main_filter_shop_list = $this->db->select('city.*')->from('city')->where('city.is_deleted','0')->where('city.state_id',$all_shop_id)->get()->result();
        //print_r ($main_filter_shop_list[0]->service_id);
       echo json_encode($main_filter_shop_list);
  ...
  public function edit_shop($id1) {
    ...
    public function insert_shop(){
      if ($this->input->post() && $this->input->post() != '') {
        ...
        $user_list = $this->general_model->get_user_data('*', 'user', array('id' => $uidd, 'is_deleted' => 0));
         $this->data['userlist'] = $user_list;
		...
		$shop_count = $this->general_model->check_exist_data('id', 'shop', array('shop_name' => $_POST['shop_name'], 'user_id' => $uid));
		if($shop_count < 5){
		 $shop_countt = $this->general_model->check_exist_data('id', 'shop', array('shop_name' => $_POST['shop_name'], 'city' => $_POST['city']));	
		 if(!empty($shop_countt)){
        $insert_id = $this->general_model->insert_shop_data($data, 'shop');
        // $shop_id = $insert_id;
        ...
        if($insert_id){
		 ...
		 {
			 $this->session->set_flashdata('error_message', "Can't  add same shop name in same city .");
                 redirect('shop/add_shop', 'refresh');
		 }
		...
		{
		   $this->session->set_flashdata('error_message', "Sorry, Can't add more than five city");
           redirect('shop/add_shop', 'refresh');
		}
    ...
    public function upload_muliple_image($shop_id, $json_files2)
   {
    ...
    public function update_shop_back($id1){
    ...
    public function update_shop($id1){
      ...
      if ($this->input->post() && $this->input->post() != '') {
       ...
       // echo $categories_final;exit;
        ...
        // echo $end_time; exit;
        $shop_count = $this->general_model->check_exist_data('id', 'shop', array('id != ' => $shop_id, 'shop_name' => $_POST['shop_name'], 'is_deleted' => '0'));
        // echo $shop_count;exit;
        if(empty($shop_count)) {
            $shop_city_count = $this->general_model->check_exist_data('id', 'shop', array('id != ' => $shop_id, 'shop_name' => $_POST['shop_name'], 'city' => $_POST['city_id'], 'is_deleted' => '0'));
            // echo $shop_count;exit;
            if(empty($shop_count)) {
                $check_shop_name = $this->general_model->check_exist_data('id,shop_name', 'shop', array('id' => $shop_id));
                // echo "<pre>"; print_r($check_shop_name->shop_name);exit;
                if($check_shop_name->shop_name != $_POST['shop_name']) {
                    $data['changed_shop_name'] = $_POST['shop_name'];
                    $data['is_shop_name_approved'] = 1;
                } else {
                ...
                $data['service_id'] = $categories_final;
                $data['worker_id'] = $workers;
                $data['start_time'] = $start_time;
                $data['end_time'] = $end_time;
                $data['shop_email'] = $_POST['shop_email'];
                $data['mobile'] = $_POST['mobile_no'];
                $data['addline1'] = $_POST['address_1'];
                $data['addline2'] = $_POST['address_2'];
                $data['city'] = $_POST['city_id'];
                $data['state'] = $_POST['state'];
                $data['zipcode'] = $_POST['zipcode'];
                $data['description'] = $_POST['discription'];
                $data['cancel_policy'] = $_POST['radiog_dark'];
                $data['image'] = $image;
                ...
                $this->session->set_flashdata('error_message', "Duplicate same shop name with same city not allowed!");
                redirect('shop/edit_shop/'.$id1, 'refresh');
            }
        } else {
            $this->session->set_flashdata('error_message', "Duplicate Shop name not allowed!");
            redirect('shop/edit_shop/'.$id1, 'refresh');
        }
        // echo "<pre>"; print_r($categories); exit;
      } else {
