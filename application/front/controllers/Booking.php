...
class Booking extends MY_Controller {
    ...
    public function index() {
      ...
      //print_r($this->session->userdata('email')); 
       ...
       $shop_permission = $worker[0]->shop_permission;
       if($shop_permission=='1')
       {
          $user_list1 = $this->general_model->get_user_data('*', 'user', array('id' => $id, 'is_active' => 1));
          $this->data['userlist'] = $user_list1;
          $worker_id = $user_list1->p_id; 
       }
       else{
            $worker_id = $worker_id;
       }
       ...
       if($usertype=='3'){
        ...
        $past_booking_data = $this->general_model->get_order_booking_data('*', 'appointment', ("worker_id= '".$worker_id."'  AND ap_date < CURRENT_DATE() AND is_deleted = 0"));
        }
         elseif($usertype=='2'){
         
        $query = $this->db->select('id');
        $query = $this->db->get_where('shop', array('user_id' =>$this->session->userdata('uid')));
        $shop = $query->result();
        $items = array();
        $i=0;
        foreach($shop as $username) {
         $items[] = $username->id;
        $i++;}
        $shop_id = $items;
        // $past_booking_data = $this->general_model->get_order_booking_data_new('*', 'appointment', ("(booking_status = 2 OR booking_status = 3) AND ap_date < CURRENT_DATE() AND is_deleted = 0"),$shop_id);
         $past_booking_data = $this->general_model->get_order_booking_data_new('*', 'appointment', ("ap_date < CURRENT_DATE() AND is_deleted = 0"),$shop_id);
        ...
        else
        {
            // $past_booking_data = $this->general_model->get_order_booking_data('*', 'appointment', ("user_id= '".$this->session->userdata('uid')."'  AND (booking_status = 2 OR booking_status = 3) AND ap_date < CURRENT_DATE() AND is_deleted = 0"));
             ...
             $past_booking_data = $this->general_model->get_order_booking_data('*', 'appointment', ("user_id= '".$this->session->userdata('uid')."'  AND ap_date < CURRENT_DATE() AND is_deleted = 0"));
        }
        ...
        $present_booking_data = $this->general_model->get_order_booking_data_new('*', 'appointment', (" (booking_status = 0 OR booking_status = 1) AND ap_date >= CURRENT_DATE() AND is_deleted = 0"),$shop_id);
        ...
        $finished_booking_data = $this->general_model->get_order_booking_data_new('*', 'appointment', (" (booking_status = 2)  AND is_deleted = 0"),$shop_id);
    ...
    public function booking_data()
    {
      ...
      if(isset($start, $limit))
      {
        ...
        // $appointment_list = $this->general_model->get_appointment_all_data( 'appointment', ("appointment.worker_id= '".$worker_id."' AND (appointment.booking_status = 2 OR appointment.booking_status = 3) AND appointment.ap_date < CURRENT_DATE() AND appointment.is_deleted = 0"),$limit,$start);
            ...
            $appointment_list = $this->general_model->get_appointment_all_data( 'appointment', ("appointment.worker_id= '".$worker_id."'  appointment.ap_date < CURRENT_DATE() AND appointment.is_deleted = 0"),$limit,$start);
          }
      elseif($usertype=='2'){
         
        $query = $this->db->select('id');
        $query = $this->db->get_where('shop', array('user_id' =>$id));
        $shop = $query->result();
        $items = array();
        $i=0;
        foreach($shop as $username) {
        ...
        $i++;}
         $shop_id = $items;
        // $appointment_list = $this->general_model->get_appointment_all_data_new( 'appointment', ('(appointment.booking_status = 2 OR appointment.booking_status = 3)  AND appointment.is_deleted = 0'),$shop_id,$limit,$start);

          $appointment_list = $this->general_model->get_appointment_all_data_new( 'appointment', ('appointment.is_deleted = 0'),$shop_id,$limit,$start);
            }
          else{
          // $appointment_list = $this->general_model->get_appointment_all_data( 'appointment', ("appointment.user_id= '".$this->session->userdata('uid')."' AND (appointment.booking_status = 2 OR appointment.booking_status = 3) AND appointment.ap_date < CURRENT_DATE() AND appointment.is_deleted = 0"),$limit,$start);
            ...
            $appointment_list = $this->general_model->get_appointment_all_data( 'appointment', ("appointment.user_id= '".$this->session->userdata('uid')."' AND appointment.ap_date < CURRENT_DATE() AND appointment.is_deleted = 0"),$limit,$start);
          }
        }
       ...
       elseif($usertype=='2'){
        ...
        $i++;}
         $shop_id = $items;
      $appointment_list = $this->general_model->get_appointment_all_data_new( 'appointment', ('(appointment.booking_status = 0 OR appointment.booking_status = 1) AND appointment.ap_date >= CURRENT_DATE()  AND appointment.is_deleted = 0'),$shop_id,$limit,$start);
            }
          else{
       ...
       elseif($usertype=='2'){
        ...
        $i++;}
         $shop_id = $items;
      $appointment_list = $this->general_model->get_appointment_all_data_new( 'appointment', ('appointment.booking_status = 2  AND appointment.is_deleted = 0'),$shop_id,$limit,$start);
            }
          else{
        ...
        if(count($appointment_list) > 0){
          foreach ($appointment_list as $key => $appointment) {
           ...
           // if($appointment->mainuser != $id){
              ...
              if($user_category == 1){
                $html .= '<div class="button_link col-md-3"><a href="'.site_url().'appointment/appointment_step1/'.$appointment->shop_id.'/'.$encrypt_id.'">BOOK AGAIN</a></div><div class="button_link col-md-3"><a href="void:javascript(0)" class="btn_call_us" data-shop-mobile="'.$appointment->shop_mobile.'">CALL US</a></div>';
                if($appointment->refund_request=='no'){
                ...
                elseif ($appointment->refund=='yes') {
                  $html .= '<div class="col-md-2 button_link  col-md-3'.$appointment->id.'"><a href="void:javascript(0)" class="btn_refund" data-appointment-id="'.$appointment->id.'">REFUND COMPLETE</a></div>';
                }
                elseif ($appointment->refund=='no' && $appointment->refund_request=='yes' ) {
                  $html .= '<div class="col-md-2 button_link col-md-3'.$appointment->id.'"><a href="void:javascript(0)" class="btn_refund" data-appointment-id="'.$appointment->id.'">REFUND PENDING</a></div>';
                }
           ...
           if($user_category == 1){
            ...
            if($appointment->refund_request=='no'){
                ...
                elseif ($appointment->refund=='yes') {
                  $html .= '<div class="col-md-2 button_link  col-md-3'.$appointment->id.'"><a href="void:javascript(0)"  data-appointment-id="'.$appointment->id.'">REFUND COMPLETE</a></div>';
                }
                elseif ($appointment->refund=='no' && $appointment->refund_request=='yes' ) {
                  $html .= '<div class="col-md-3 button_link col-md-3'.$appointment->id.'"><a href="void:javascript(0)"  data-appointment-id="'.$appointment->id.'">REFUND PENDING</a></div>';
                }
                // $html .= '<a href="void:javascript(0)" style="color:#ffffff" class="btn_refund" data-appointment-id="'.$appointment->id.'"><div class="col-md-2 button_link refund_ap col-md-3'.$appointment->id.'">REFUND</div></a>';
       }
              }
               if($appointment->booking_status == 0 && $appointment->ap_date >= $todaysdate){
